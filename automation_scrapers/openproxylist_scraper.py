...
def scrape_from_openproxylist(sb: BaseCase, browser_lock: threading.Lock, verbose: bool = True) -> List[str]:
    ...
    try:
        ...
        # Wait for the element containing the script to be present
        time.sleep(2)
        sb.wait_for_element_present('script[src*="recaptcha/api.js?render="]', timeout=20)
        # Wait for the container, then find the script

        # sb.wait_for_element_present('.container-fluid > script[src*="recaptcha"]', timeout=20)

        time.sleep(1) # Extra delay for JS to fully initialize
