...
def main():
    ...
    try:
        ...
        if not urls_to_scrape:
             ...
             # We use a lambda to pass the arguments to the function
             scraper_tasks[f'Websites ({SITES_FILE})'] = lambda: scrape_proxies(urls_to_scrape, verbose=True)
    ...
    print(f"--- Starting {len(scraper_tasks)} scrapers concurrently ---")
    ...
    print("\n--- Summary ---")
    # Print the summary in a consistent order
    for name in sorted(results.keys()):
        print(f"Found {len(results[name])} proxies from {name}.")
